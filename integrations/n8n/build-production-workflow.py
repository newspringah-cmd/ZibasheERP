...
nodes = [
  ...
  {"id":"telegram","name":"Send PDF To Customer Group","type":"n8n-nodes-base.httpRequest","typeVersion":4.2,"position":[1440,-80],"parameters":{"method":"POST","url":"https://erp.zibashe.ir/api/integrations/n8n/telegram-invoices","sendHeaders":True,"headerParameters":{"parameters":[{"name":"X-Api-Key","value":"={{ $env.N8N_ZIBASHE_API_KEY }}"}]},"sendBody":True,"contentType":"multipart-form-data","bodyParameters":{"parameters":[{"parameterType":"formData","name":"sourceEventId","value":"={{ $('Build Invoice HTML').first().json.eventId }}"},{"parameterType":"formData","name":"chatId","value":"={{ $('Build Invoice HTML').first().json.invoiceDeliveryChatId }}"},{"parameterType":"formData","name":"caption","value":"={{ $('Build Invoice HTML').first().json.telegramCaption }}"},{"parameterType":"formBinaryData","name":"document","inputDataFieldName":"data"}]},"options":{}}},
