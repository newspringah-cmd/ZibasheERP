...
nodes = [
  ...
  {"id":"telegram","name":"Send PDF To Customer Group","type":"n8n-nodes-base.telegram","typeVersion":1.2,"position":[1440,-80],"parameters":{"resource":"message","operation":"sendDocument","chatId":"={{ $('Build Invoice HTML').first().json.data.Delivery.ChatId }}","binaryData":True,"binaryPropertyName":"data","replyMarkup":"inlineKeyboard","inlineKeyboard":{"rows":[{"row":{"buttons":[{"text":"✅ پرداخت‌شده","additionalFields":{"callback_data":"={{ $('Build Invoice HTML').first().json.paidCallbackData }}"}},{"text":"⏳ در انتظار پرداخت","additionalFields":{"callback_data":"={{ $('Build Invoice HTML').first().json.waitingCallbackData }}"}}]}},{"row":{"buttons":[{"text":"={{ $('Build Invoice HTML').first().json.firstCardCopyLabel }}","additionalFields":{"copy_text":{"text":"={{ $('Build Invoice HTML').first().json.firstCardCopyText }}"}}}]}},{"row":{"buttons":[{"text":"={{ $('Build Invoice HTML').first().json.secondCardCopyLabel }}","additionalFields":{"copy_text":{"text":"={{ $('Build Invoice HTML').first().json.secondCardCopyText }}"}}}]}}]},"additionalFields":{"caption":"={{ $('Build Invoice HTML').first().json.telegramCaption }}"}}},
...
workflow = {"id":"zibashe-production-events","name":"Zibashe Production Events","nodes":nodes,"pinData":{},"connections":{
  ...
  "Send PDF To Customer Group":{"main":[[{"node":"Acknowledge Invoice Delivery","type":"main","index":0},{"node":"Build Artifact Callback","type":"main","index":0}]]},
