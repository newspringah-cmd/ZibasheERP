...
nodes = [
  ...
  {"id":"delivery-if","name":"Has Customer Delivery","type":"n8n-nodes-base.if","typeVersion":2.2,"position":[840,-80],"parameters":{"conditions":{"options":{"caseSensitive":True,"leftValue":"","typeValidation":"strict","version":2},"conditions":[{"id":"delivery-condition","leftValue":"={{ $json.isManualReview }}","rightValue":False,"operator":{"type":"boolean","operation":"equals"}}],"combinator":"and"},"options":{}}},
  ...
  {"id":"telegram","name":"Send PDF To Customer Group","type":"n8n-nodes-base.httpRequest","typeVersion":4.2,"position":[1440,-80],"parameters":{"method":"POST","url":"https://erp.zibashe.ir/api/integrations/n8n/telegram-invoices","sendHeaders":True,"headerParameters":{"parameters":[{"name":"X-Api-Key","value":"={{ $env.N8N_ZIBASHE_API_KEY }}"}]},"sendBody":True,"contentType":"multipart-form-data","bodyParameters":{"parameters":[{"parameterType":"formData","name":"sourceEventId","value":"={{ $('Build Invoice HTML').first().json.eventId }}"},{"parameterType":"formData","name":"chatId","value":"={{ $('Build Invoice HTML').first().json.invoiceDeliveryChatId }}"},{"parameterType":"formData","name":"caption","value":"={{ $('Build Invoice HTML').first().json.telegramCaption }}"},{"parameterType":"formData","name":"fileName","value":"={{ $('Build Invoice HTML').first().json.invoiceFileName }}"},{"parameterType":"formBinaryData","name":"document","inputDataFieldName":"data"}]},"options":{}}},
  {"id":"respond","name":"Acknowledge Invoice Delivery","type":"n8n-nodes-base.respondToWebhook","typeVersion":1.4,"position":[1680,-180],"parameters":{"respondWith":"json","responseBody":"={{ { \"ok\": true, \"delivered\": !$('Build Invoice HTML').first().json.isManualReview } }}","options":{"responseCode":200}}},
...
workflow = {"id":"zibashe-production-events","name":"Zibashe Production Events","nodes":nodes,"pinData":{},"connections":{
  ...
  "Build Invoice HTML":{"main":[[{"node":"Has Customer Delivery","type":"main","index":0}]]},
  "Has Customer Delivery":{"main":[[{"node":"HTML To Binary","type":"main","index":0}],[{"node":"Acknowledge Invoice Delivery","type":"main","index":0}]]},
