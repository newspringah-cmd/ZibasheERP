...
public sealed partial class TelegramWebhookController
{
        ...
        string? topNotes, string? middleNotes, string? baseNotes, bool compact)
    {
        ...
        if (values.Length == 1)
            return $"🌸 نت‌ها: {Html(values[0].Value)}\n\n";
        var lines = values.Select(item => compact
            ? $"{item.Emoji} {Html(item.Value)}"
            : $"{item.Emoji} {item.Label}: {Html(item.Value)}");
        return string.Join(compact ? " | " : "\n\n", lines) + "\n\n";
    }

    private static string BuildStackedUserLines(string label, string[] users) =>
        $"{label}:\n\n{string.Join("\n", users)}";
    ...
    private static string FancyBottleMarker(Bottle? bottle)
    {
        if (bottle?.Type != BottleType.Fancy) return string.Empty;
        var name = bottle.Name;
        if (name.Contains("silver", StringComparison.OrdinalIgnoreCase)) return " F silver";
        if (name.Contains("red", StringComparison.OrdinalIgnoreCase)) return " F red";
        if (name.Contains("gold", StringComparison.OrdinalIgnoreCase)) return " F gold";
        return " F";
    }
