...
public sealed partial class TelegramWebhookController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (await TryHandleShippingPreparationCommandAsync(message, cancellationToken))
            return true;

        if (await TryHandleShippingPreparationMessageAsync(message, cancellationToken))
            return true;

        if (await TryHandleSalesListImportEditMessageAsync(message, cancellationToken))
            return true;
        ...
        if (await TryHandleInvoiceResendMessageAsync(message, cancellationToken))
            return true;
        if (await TryHandleCompletedListEditMessageAsync(message, cancellationToken))
            return true;
        if (await TryHandleCompletedListResendMessageAsync(message, cancellationToken))
            return true;
