...
## قواعد لیست فروش کانال

- لیست فروش سفارش‌محور است و پیش از خرید عطر، مستقیماً برای یک `Perfume` باز می‌شود؛ `Batch` واقعی بعد از خرید متصل می‌شود.
- صاحب باتل می‌تواند هنگام بازکردن لیست ثبت شود یا موقتاً نامشخص بماند. مقدار او از ظرفیت همان لیست کم می‌شود و هزینه شیشه‌اش صفر است.
- صف `Next Bottle` فقط توسط ادمین مدیریت می‌شود. ارتقای یک نفر به صاحب باتل، او را از صف خارج و به درخواست جاری منتقل می‌کند.
- درخواست کانال می‌تواند برای خود کاربر یا هدیه باشد. هدیه، هویت هدیه‌دهنده و هدیه‌گیرنده را جداگانه نگه می‌دارد و در لیست با قالب `giver for recipient` نمایش داده می‌شود.
- هدیه به صاحب باتل هزینه شیشه ندارد؛ سایر هدیه‌ها طبق شیشه نرمال یا فانتزی قیمت‌گذاری می‌شوند.
- دریافت شناسه هدیه‌گیرنده برای مشتریان در Discussion Group متصل به کانال انجام می‌شود و نیازی به شروع گفت‌وگوی خصوصی ربات ندارد.
- تغییرات دستی صاحب باتل، صف و سفارش‌ها باید در گروه Audit فروش ثبت شوند.
...
فایل‌های پایه استقرار VPS در پوشه `deploy` قرار دارند. API داخل کانتینر non-root اجرا و فقط روی loopback سرور منتشر می‌شود تا TLS و دسترسی عمومی توسط reverse proxy کنترل شوند. SQL Server و n8n پس از مشخص‌شدن RAM، CPU، فضای دیسک، سیستم‌عامل و سیاست backup سرور به Compose تولید اضافه یا به سرویس جدا متصل می‌شوند.
