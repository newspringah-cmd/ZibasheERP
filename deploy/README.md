...
قالب Caddy برای سرور تازه آماده است و TLS را به‌صورت خودکار مدیریت می‌کند. اگر VPS از قبل nginx و Certbot معتبر دارد، همان reverse proxy حفظ می‌شود؛ `erp.zibashe.ir` باید به `http://127.0.0.1:8080` و `n8n.zibashe.ir` به `http://127.0.0.1:5678` متصل شوند. روی یک Host فقط یکی از nginx یا Caddy مالک پورت‌های 80 و 443 است.

برای سرور تازه مبتنی بر Caddy، پس از آنکه DNS هر دو دامنه به IP سرور اشاره کرد، فایل نهایی را بدون واردکردن scheme یا مسیر بسازید:
...
کلید OpenAI نباید در Git یا لاگ‌ها ثبت شود. خروجی شامل متن اصلی ثبت‌شده برای مقایسه مسئول ارسال، یک عکس پیش‌نمایش و یک PDF تک‌صفحه‌ای افقی با اندازه دقیق ۸۰ در ۵۰ میلی‌متر و فونت B Nazanin جاسازی‌شده است. پاسخ دکمه ارسال بلافاصله پس از ثبت درخواست پست داده می‌شود و کاربر منتظر پردازش OpenAI و تولید فایل نمی‌ماند؛ خطای احتمالی لیبل همراه آیدی مشتری و شناسه درخواست، هم در مبدأ عملیات و هم در گروه آماده‌سازی بسته گزارش می‌شود. اندازه متن برای آدرس‌های معمولی بزرگ‌تر و برای آدرس‌های بلند به‌صورت خودکار متناسب می‌شود. نام گیرنده در هر جای متن خام (ابتدا، وسط یا انتها) قابل استخراج است، می‌تواند فقط یک نام خانوادگی تک‌کلمه‌ای باشد و نام پیش‌فرض مشتری به متن خام تحمیل نمی‌شود. مقصدهای چندسطحی با همه سطوح موجود، مانند «فارس، داراب، دولت‌آباد»، در خط «شهر مقصد» می‌آیند و نام‌های استان/شهرستان/شهر از متن نشانی حذف می‌شوند تا تکرار نشوند. همه شماره‌های تماس متمایز، شامل موبایل و تلفن ثابت، در خط تلفن درج می‌شوند. نام گیرنده، نشانی، حداقل یک شماره تماس و شهر مقصد الزامی‌اند؛ کدپستی اختیاری است و فقط اگر دقیقاً ۱۰ رقم باشد چاپ می‌شود. آدرس‌های قدیمی که فیلدهای ساختاریافته ناقص دارند از مجموع متن موجود دوباره پردازش می‌شوند. اگر یکی از اطلاعات الزامی قابل استخراج نباشد، PDF ساخته نمی‌شود و پیام اصلاح اطلاعات برای حسابدار ارسال می‌شود. مدیر یا حسابدار می‌تواند از منوی مدیریت «ثبت دستی آدرس» را انجام دهد و «ارسال برای پست» را نیز خارج از گروه اختصاصی مشتری اجرا کند؛ در این حالت ربات ابتدا آیدی مشتری را می‌پرسد.

پیام درخواست در گروه آماده‌سازی بسته دکمه‌های «ارسال شد» و «ارسال کد رهگیری» دارد. مسئول ارسال با دکمه دوم عکس بعدی را به همان مشتری اختصاص می‌دهد و ربات آن را به همه گروه‌های فعال متصل به مشتری ارسال می‌کند؛ پس از ارسال موفق، متن همان دکمه به «کد ارسال شد» با تیک سبز تغییر می‌کند. `/cancel` انتظار عکس را لغو می‌کند.
